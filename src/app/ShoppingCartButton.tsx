  ...
  return (
    <>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent className="flex flex-col sm:max-w-lg p-5">
          <SheetHeader >
            <SheetTitle>
              You cart{" "}
            ...
            <ul className="space-y-5">
              {cartQuery.data?.lineItems?.map((item) => (
                <ShoppingCartItems
                  key={item._id}
                  item={item}
                  onProductLinkClincked={() => setSheetOpen(false)}
                />
              ))}
            </ul>
            {cartQuery.isPending && (
          ...
          <div className="flex items-center justify-between gap-5 ">
            <div className="space-y-0.5">
              ...
              <p className="font-bold">
                {/* @ts-expect-error: subtotal may be undefined or not typed as expected */}
                {cartQuery.data?.subtotal?.formattedConvertedAmount}
              </p>
              <p className="text-muted-foreground text-sm">
                Shipping and texes calculated at chackout
              </p>
            </div>
...
interface ShoppingCartItemsProps {
  ...
  onProductLinkClincked: () => void;
}

function ShoppingCartItems({
  item,
  onProductLinkClincked,
}: ShoppingCartItemsProps) {
  const updateQuantityMutaion = useUpdateCartItemQuantity();
  const removeItemMutaion = useRemoveCartItem();
  ...
  return (
    <li className="flex items-center gap-3">
      <div className="relative size-fit flex-none">
        <Link href={`/products/${slug}`} onClick={onProductLinkClincked}>
          <WixImage
        ...
        <button
          className="bg-background absolute -top-1 -right-1 cursor-pointer rounded-full border p-0.5"
          onClick={() => removeItemMutaion.mutate(productId)}
        >
          <X className="size-3" />
        </button>
      </div>
